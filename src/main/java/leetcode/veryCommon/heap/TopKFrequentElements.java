...
public class TopKFrequentElements {
    ...
    public static int[] topKFrequent(int[] nums, int k) {
        ...
        quickSelect(inpArr, 0, inpArr.length - 1, k);
        return Arrays
                .stream(Arrays.copyOfRange(inpArr, inpArr.length - k, inpArr.length))
    ...
    public static int quickSelect(int[][] inpArr, int start, int end, int k) {
        ...
        int expectedPost = inpArr.length - k;
        System.out.println(pivotPos);
        if (expectedPost == pivotPos)
            return pivotPos;
        else if (pivotPos > expectedPost)
