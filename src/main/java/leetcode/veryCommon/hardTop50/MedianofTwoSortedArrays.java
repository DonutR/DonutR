...
public class MedianofTwoSortedArrays {
    ...
    public double findMedianSortedArrays1(int[] nums1, int[] nums2) {
        ...
        int[] X = nums1, Y = nums2;
        int low = 0, xLen = X.length, yLen = Y.length, high = xLen;
        while (low <= high) {
            int xMid = (low + high) / 2;
            int yMid = ((xLen + yLen + 1) / 2) - xMid;

            int xLeftMax = xMid == 0 ? Integer.MIN_VALUE : X[xMid - 1];
            int xRightMin = xMid == xLen ? Integer.MAX_VALUE : X[xMid];
            int yLeftMax = yMid == 0 ? Integer.MIN_VALUE : Y[yMid - 1];
            int yRightMin = yMid == yLen ? Integer.MAX_VALUE : Y[yMid];

            if (xLeftMax <= yRightMin && yLeftMax <= xRightMin)
                if ((yLen + xLen) % 2 == 0)
                    return ((double) Math.max(xLeftMax, yLeftMax) + Math.min(xRightMin, yRightMin)) / 2;
                else return (double) Math.max(xLeftMax, yLeftMax);
